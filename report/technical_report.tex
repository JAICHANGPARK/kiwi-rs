...
To position \texttt{kiwi-rs} in the broader Korean NLP landscape, we organize representative prior work into eight tracks (20 papers total):
\begin{itemize}[leftmargin=1.5em]
...
These studies primarily evaluate model quality and benchmark accuracy. \texttt{kiwi-rs} is complementary: it contributes a systems-level engineering layer for safe, reproducible, and high-throughput Rust deployment over the existing Kiwi analyzer.
...
\paragraph{Reviewer-oriented disambiguation (\texttt{TypoTransformer} question).}
