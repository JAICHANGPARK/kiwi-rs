...
\begin{table*}[t]
\centering
\caption{Korean morphological analyzer landscape (engineering comparison, snapshot: 2026-02-17).}
...
Here, ``fingerprint'' denotes a fast pre-filter signature (length + boundary-byte sketch), not a collision-free identity hash; cache acceptance always requires full key equality checks.
...
\begin{enumerate}[leftmargin=1.5em]
    \item \textbf{Startup optimization track.} Evaluate constructor-path policies for the \texttt{new} and \texttt{from\_config} initialization APIs, plus prewarmed workers and pinning defaults. Report median startup deltas on Tier A/Tier B.
