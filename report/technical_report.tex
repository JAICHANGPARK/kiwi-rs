...
\author{
Jai-Chang Park\thanks{Google Developer Expert (GDE), Dart \& Flutter, Google Developer Groups (GDG) Golang Korea, Flutter Seoul}
}
\date{\today}
...
To position \texttt{kiwi-rs} in the broader Korean NLP landscape, we separate prior work into (i) academic model/benchmark context and (ii) production ecosystem context.
\paragraph{Academic model and benchmark context.}
\begin{itemize}[leftmargin=1.5em]
    \item \textbf{Korean evaluation benchmarks.} KLUE defines a multi-task Korean language understanding benchmark suite, while KorQuAD provides a Korean machine-reading comprehension benchmark~\cite{klue2021,korquad2019}.
    \item \textbf{Korean pre-trained language models.} KR-BERT and KoreALBERT report Korean-specific pretraining strategies and efficiency trade-offs for encoder models~\cite{krbert2020,korealbert2021}.
    \item \textbf{Transformer pretraining baselines.} BERT, RoBERTa, ALBERT, ELECTRA, and T5 form widely used encoder/text-to-text pretraining baselines that Korean adaptations commonly compare against~\cite{bert2018,roberta2019,albert2019,electra2020,t52020}.
    \item \textbf{Multilingual transfer models.} XLM-R and mT5 provide strong multilingual baselines that include Korean and are frequently used as cross-lingual reference points~\cite{xlmr2019,mt52020}.
    \item \textbf{Tokenization infrastructure.} SentencePiece and BPE-style subword segmentation remain central tokenization choices in practical Korean NLP pipelines~\cite{sentencepiece2018,bpe2016}.
    \item \textbf{Transformer architecture foundation.} The original Transformer formulation remains the architectural basis for most of the model families above~\cite{transformer2017}.
    \item \textbf{Sequence labeling foundations.} CRF-based structured prediction and early unified neural tagging frameworks provide important methodological context for POS/morphological sequence decisions~\cite{lafferty2001crf,collobert2011,peters2018elmo}.
    \item \textbf{Subword and OOV robustness.} Neural LM and subword representation lines (including WordPiece-style segmentation in Japanese/Korean voice search and subword-enriched embeddings) motivate robust handling of sparse or unseen forms~\cite{bengio2003nlm,schuster2012jkvoice,bojanowski2017subword}.
\end{itemize}

\paragraph{Production ecosystem context.}
For practical deployment choices, the current Korean morphology landscape includes open-source analyzers and hosted services.
Representative options include KoNLPy's multi-analyzer wrapper (\texttt{Hannanum}/\texttt{Kkma}/\texttt{Komoran}/\texttt{Mecab}/\texttt{Okt}), MeCab-ko lineage analyzers, Kakao's Khaiii, KOMORAN, Open Korean Text, Elasticsearch Nori integration, and hosted APIs such as Bareun/ETRI platforms~\cite{konlpy_docs,konlpy_tag_api,konlpy_paper,mecab_ko,khaiii_repo,komoran_repo,openkoreantext_repo,mecab2004crf,nori_ko,bareun_pypi,etri_aiopen_notice,etri_epretx,etri_epretx_api_list}. Table~\ref{tab:ko_analyzer_landscape} summarizes these options from an engineering perspective.

These two bodies of work are complementary to \texttt{kiwi-rs}: the academic line informs model-level context, while the ecosystem line clarifies real deployment alternatives.

Table~\ref{tab:ko_analyzer_landscape} summarizes widely used Korean morphological analysis options from an engineering deployment perspective.

\begin{table*}[t]
\centering
\caption{Korean morphological analyzer landscape (engineering comparison, snapshot: 2026-02-18).}
\label{tab:ko_analyzer_landscape}
\scriptsize
\renewcommand{\arraystretch}{1.12}
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}P{0.11\linewidth}P{0.19\linewidth}P{0.16\linewidth}P{0.22\linewidth}P{0.24\linewidth}@{}}
\toprule
System & Core model/runtime & Distribution model & Interfaces & Operational notes \\
\midrule
Kiwi~\cite{kiwi_repo,kiwi_kjdh_2024,kiwi_releases} & C++ analyzer with statistical LM + skip-bigram disambiguation & Open-source library with prebuilt binaries for major OSes & C API; wrappers for Python, Java, C\#, Go, R, and WASM & Broad ecosystem and rich POS extensions; baseline engine targeted by \texttt{kiwi-rs} \\

Khaiii~\cite{khaiii_repo} & Data-driven CNN-based Korean morphological analyzer & Open-source project repository & Native runtime and tooling documented in the upstream project & Upstream docs describe dictionary and patch workflows for practical correction loops \\

KOMORAN & Java-based analyzer with dictionary-centered design~\cite{komoran_repo} & Open-source Java library & Java API; ecosystem wrappers (e.g., PyKOMORAN) & JVM-native deployment; custom dictionary workflow emphasized \\

Open Korean Text (Okt)~\cite{openkoreantext_repo,konlpy_tag_api} & Scala/Java Korean text processor & Open-source library & Scala/Java APIs; KoNLPy \texttt{Okt} wrapper & Supports normalization, tokenization, stemming, and phrase extraction for practical indexing workflows \\

MeCab-ko lineage~\cite{mecab_ko,mecab2004crf,konlpy_tag_api} & MeCab-style dictionary analyzer lineage rooted in CRF-era segmentation/POS design & Community distributions (\texttt{mecab-ko} and dictionaries) & Standalone deployment; KoNLPy \texttt{Mecab} wrapper & Common production choice in indexing/search stacks when dictionary control is required \\

KoNLPy wrapper layer~\cite{konlpy_docs,konlpy_paper,konlpy_tag_api} & Python integration layer over heterogeneous analyzers & Python package & Unified API over Hannanum, Kkma, Komoran, Mecab, and Okt & Strong prototyping ergonomics; runtime behavior depends on selected backend analyzer \\

Elasticsearch Nori~\cite{nori_ko} & Lucene Nori module using \texttt{mecab-ko-dic} & Elasticsearch analysis plugin & Analyzer components for Elasticsearch index/query pipelines & Search-oriented morphological analysis integration for production retrieval systems \\

Bareun API~\cite{bareun_pypi} & Hosted Korean NLP service (\texttt{bareunpy} gRPC client package) & Managed service with client SDK & Python client and service endpoint integration & Provider-managed API operation with credential-based access \\

ETRI EPRETX~\cite{etri_aiopen_notice,etri_epretx,etri_epretx_api_list} & Public hosted AI API platform (language-processing category) & Managed service portal & Key-based API access from EPRETX portal & AI Open site reports closure on 2025-06-30; EPRETX provides the current portal/API list \\
\bottomrule
\end{tabular}
\end{table*}

\section{Background: Kiwi}
Kiwi is a Korean morphological analyzer implemented in C++ and distributed with a public C API, which makes it usable across multiple language runtimes~\cite{kiwi_repo}. \textbf{In modeling terms, Kiwi is not presented upstream as a Transformer architecture; it is described as statistical LM + skip-bigram based disambiguation}~\cite{kiwi_repo,kiwi_kjdh_2024}. In practice, Kiwi is commonly consumed through \texttt{kiwipiepy} in Python workflows, while additional ecosystem bindings target other environments~\cite{kiwipiepy,kiwi_repo}.

\subsection{Modeling Basis Clarification (Not Transformer Architecture)}
Because the Kiwi codebase contains identifiers such as \path{TypoTransformer}, it is easy to misread the engine as Transformer NLP. The same risk appears with names like \path{history_transformer}. Upstream documentation and the project citation, however, describe Kiwi's ambiguity resolution as statistical language-model plus skip-bigram disambiguation~\cite{kiwi_repo,kiwi_kjdh_2024}. In this report, we therefore treat Kiwi as a statistical morphology analyzer with LM-based disambiguation, while Transformer-family papers are used only as adjacent related-work context.

\paragraph{Architectural Clarification: The \texttt{TypoTransformer} Component.}
For the specific review concern ``\texttt{TypoTransformer} exists, so is Kiwi Transformer-based?'', our source audit indicates \textbf{no}:
\begin{itemize}[leftmargin=1.5em]
    \item \texttt{TypoTransformer} is an error/typo generation-correction utility class (rule/cost based), not a neural encoder block.
    \item \texttt{history\_transformer} in KNLM code is a history-index remapping argument for n-gram construction, not self-attention computation.
    \item Core LM implementation units are explicitly named \texttt{Knlm} and \texttt{CoNgramModel}; skip-bigram behavior is represented by \texttt{SkipBigramModel}, consistent with statistical LM design.
    \item Build configuration in the upstream project does not introduce deep-learning runtime dependencies.
\end{itemize}
...
\subsection{Kiwi Architecture (Conceptual View)}
Figure~\ref{fig:kiwi_architecture} summarizes a conceptual inference flow for Kiwi based on upstream descriptions (statistical LM + skip-bigram), public C API boundaries, and source-level module naming~\cite{kiwi_repo,kiwi_kjdh_2024}. This is an explanatory architecture map, not a line-by-line reconstruction of private internal call graphs.
...
\node[box, fill=green!10] (input) {Input Text};
\node[box, right=of input, text width=3.0cm] (candidate) {Candidate Generation\\dictionary + user lexicon\\pre-analyzed entries};
\node[box, right=of candidate, text width=3.0cm] (lattice) {Morpheme Lattice\\candidate paths};
\node[box, right=of lattice, text width=3.0cm, fill=orange!12] (scoring) {Statistical Scoring\\\texttt{Knlm} + \texttt{SkipBigramModel}};
\node[box, right=of scoring, text width=2.9cm, fill=green!10] (decode) {Path Decoding\\best analysis output};
\node[box, below=of scoring, text width=3.0cm, fill=yellow!15] (typo) {Typo Handling\\\texttt{TypoTransformer}\\rule/cost channel};
\node[box, below=of decode, text width=2.9cm, fill=purple!10] (api) {C API Output\\tokens + POS + offsets};

\draw[edge] (input) -- (candidate);
\draw[edge] (candidate) -- (lattice);
\draw[edge] (lattice) -- (scoring);
\draw[edge] (scoring) -- (decode);
\draw[edge] (decode) -- (api);
\draw[control] (typo) -- (candidate);
\draw[control] (typo) -- (scoring);
\end{tikzpicture}
}
\caption{Conceptual Kiwi architecture (upstream-aligned abstraction). Solid arrows: primary inference flow; dashed arrows: typo-control influence.}
\label{fig:kiwi_architecture}
\end{figure}

\subsection{Technologies Used by Kiwi}
From an implementation and modeling perspective, Kiwi combines several practical technologies~\cite{kiwi_repo,kiwi_kjdh_2024}:
\begin{itemize}[leftmargin=1.5em]
    \item \textbf{C++ native runtime + C API surface:} core analyzer logic is implemented natively and exported through a stable C boundary (\texttt{include/kiwi/capi.h}).
    \item \textbf{Statistical language-model scoring:} ambiguity resolution is framed as sequence scoring over morphological candidates (\texttt{Knlm} family).
    \item \textbf{Skip-bigram interaction modeling:} non-local morpheme interaction features are included through dedicated skip-bigram model components.
    \item \textbf{Typo-processing utilities:} typo generation/correction utilities (e.g., \texttt{TypoTransformer}) provide error-handling support without implying Transformer self-attention architecture.
    \item \textbf{Release-packaged deployment artifacts:} prebuilt binaries and model assets are distributed for major operating systems, enabling multi-language bindings and production integration.
\end{itemize}

\subsection{Conceptual Scoring Formulation}
To make the statistical basis explicit, we use the following \emph{conceptual} decomposition for analysis \(y=(m_1,\dots,m_T)\) of input text \(x\):
\[
\operatorname{Score}(y \mid x)
=
\lambda_{\mathrm{lm}} \sum_{t=1}^{T} \log P_{\mathrm{KNLM}}(m_t \mid h_t)
+ \lambda_{\mathrm{sb}} \sum_{t=2}^{T} s_{\mathrm{SB}}(m_{\pi(t)}, m_t)
- \lambda_{\mathrm{ty}} C_{\mathrm{typo}}(x, y)
+ \lambda_{\mathrm{lex}} B_{\mathrm{lex}}(y).
\]
The selected output is then
\[
\hat{y}=\arg\max_{y \in \mathcal{Y}(x)} \operatorname{Score}(y \mid x),
\]
where \(\mathcal{Y}(x)\) is the candidate set induced by dictionary entries, user lexicon additions, and analyzer constraints.

In this formulation, \(h_t\) denotes the KNLM history state at position \(t\), \(\pi(t)\) denotes a linked predecessor index (adjacent bigram is the special case \(\pi(t)=t-1\)), \(s_{\mathrm{SB}}(\cdot,\cdot)\) is a skip-bigram interaction score, \(C_{\mathrm{typo}}(x,y)\) is a typo-related penalty term, and \(B_{\mathrm{lex}}(y)\) is a lexicon/user-dictionary bonus. Coefficients \(\lambda_{\mathrm{lm}}, \lambda_{\mathrm{sb}}, \lambda_{\mathrm{ty}}, \lambda_{\mathrm{lex}}\) are non-negative mixture weights.

\noindent\textit{Note.} This equation is intentionally explanatory: it reflects the reported statistical-LM + skip-bigram design direction and clarifies interaction structure, while exact feature templates, predecessor linking policy, and learned/fixed weights remain implementation-specific in upstream Kiwi.
...
\subsection{Terminology: Cache and Warm Paths}
We use the following terms consistently in this report:
\begin{itemize}[leftmargin=1.5em]
    \item \textbf{Cold path:} execution on a cache miss, including full request preparation and native runtime computation.
    \item \textbf{Warm path:} execution state where reusable artifacts (cache entries, prepared join structures, pair decisions) reduce repeated work.
    \item \textbf{Cache-active varied input:} varied-input runs with a finite dataset pool; partial cache reuse can still occur.
    \item \textbf{Cache-minimized stress:} varied-input runs with a large variant pool to suppress accidental reuse and approximate lower cache benefit.
\end{itemize}
...
\subsection{Analytical Cache Model}
...
With hit latency \(\tau_{f,\mathrm{hit}}\), miss latency \(\tau_{f,\mathrm{miss}}\), and \(\kappa_f=\tau_{f,\mathrm{miss}}/\tau_{f,\mathrm{hit}} \ge 1\), expected Rust-side latency is:
\[
\tau_{f,R}^{(m)} = h_f^{(m)}\tau_{f,\mathrm{hit}} + \left(1-h_f^{(m)}\right)\tau_{f,\mathrm{miss}}
= \tau_{f,\mathrm{miss}}\!\left[\left(1-h_f^{(m)}\right)+\frac{h_f^{(m)}}{\kappa_f}\right].
\]
...
\[
A_f^{\mathrm{model}} \approx
\frac{\left(1-h_f^{(v)}\right)+h_f^{(v)}/\kappa_f}
{\left(1-h_f^{(r)}\right)+h_f^{(r)}/\kappa_f}.
\]
This model explains why \(A_f^{\mathrm{model}}\) grows when repeated-mode key diversity shrinks (\(|U_f^{(r)}| \ll |U_f^{(v)}|\)) and hit/miss latency contrast is large (\(\kappa_f \gg 1\)).

Observation scope: \(C_f\) values are directly observable from implementation constants (Table~\ref{tab:cache_design}), and \(|U_f^{(m)}|\) can be measured from benchmark key streams. In this report, however, we do not instrument per-feature cache-hit counters or direct hit/miss latency probes; therefore \(h_f^{(m)}\), \(\kappa_f\), and \(A_f^{\mathrm{model}}\) are explanatory latent quantities rather than directly estimated parameters.
...
\paragraph{Maintenance model.}
The skill package is currently maintained as version-controlled static artifacts (\texttt{SKILL.md}, references, and agent config YAML) in the same repository. Updates are release-coupled: when APIs or parity boundaries change, the skill text is revised in the same review cycle to keep prompt guidance aligned with the shipped crate surface.
...
\[
\tilde{x}_{f,e} = \operatorname{median}_{i}(x_{f,e,i}).
\]
...
\[
\hat{p}_f = \frac{1}{B}\sum_{b=1}^{B}\mathbf{1}\!\left[\rho_f^{*(b)} > 1\right].
\]
...
\begin{itemize}[leftmargin=1.5em]
    \item \texttt{Git SHA} and dirty/clean status (captured in artifact metadata fields \texttt{environment.git\_sha} and \texttt{environment.git\_dirty});
...
\setlength{\tabcolsep}{3pt}
\begin{tabular}{P{0.46\linewidth}P{0.15\linewidth}P{0.15\linewidth}P{0.15\linewidth}}
\toprule
...
Startup medians also shift under pinning. The unpinned varied-input run shows \(1326.721\) ms (\texttt{kiwi-rs}) vs \(622.918\) ms (\texttt{kiwipiepy}); the path-pinned varied-input run shows \(1461.594\) ms vs \(764.915\) ms (both \(n=5\)). This reinforces that startup comparisons should be interpreted with explicit runtime-path metadata rather than as universal constants.
...
\[
A_f^{\mathrm{emp}} = \frac{\hat{\rho}_f^{\text{repeated}}}{\hat{\rho}_f^{\text{varied}}}.
\]
Values \(A_f^{\mathrm{emp}} \gg 1\) indicate strong repeated-mode amplification relative to the varied profile.
...
\paragraph{Startup mitigation options.}
For production deployments, practical mitigation options are:
\begin{itemize}[leftmargin=1.5em]
    \item warm process pools that amortize one-time initialization;
    \item explicit path pinning via \path{KIWI_LIBRARY_PATH} and \path{KIWI_MODEL_PATH} to reduce discovery variance;
    \item explicit constructors (\texttt{Kiwi::new} or \texttt{Kiwi::from\_config}) when auto-bootstrap behavior is unnecessary;
    \item a future ``fast-init'' mode that skips discovery/bootstrap by requiring fixed runtime paths at deploy time (targeting short-lived CLI/FaaS processes).
\end{itemize}
...
\paragraph{Memory-footprint note.}
The current benchmark harness is throughput/latency oriented and does not yet report peak RSS or allocator-level memory statistics. We therefore avoid quantitative memory claims in this report and treat memory comparison as an explicit follow-up task.

\subsection{Build and Deployment Footprint}
...
\begin{table}[t]
\centering
\caption{Package and binary-size measurements (\texttt{kiwi-rs} v0.1.4, macOS arm64 snapshot).}
\label{tab:package_binary_footprint}
\begin{tabular}{P{0.40\linewidth}rP{0.40\linewidth}}
\toprule
Item & Size (bytes) & Notes \\
\midrule
\texttt{kiwi-rs-0.1.4.crate} (\texttt{cargo package --offline}) & \num{371465} & \(\approx\)362.8 KiB compressed package archive \\
Minimal release binary (baseline app) & \num{407232} & \texttt{println!} only \\
Minimal release binary (+ \texttt{kiwi-rs}, \texttt{Kiwi::init()}) & \num{576608} & dependency linked, init path referenced \\
\textbf{Binary delta (consumer app)} & \textbf{\num{169376}} & \textbf{\(+41.6\%\), \(\approx\)0.162 MiB} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Runtime payload snapshot (resolved under \texttt{\$HOME/.local/kiwi}).}
\label{tab:runtime_payload_footprint}
\begin{tabular}{P{0.46\linewidth}rP{0.28\linewidth}}
\toprule
Runtime asset & Size (bytes) & Size (MiB) \\
\midrule
\texttt{lib/libkiwi.0.22.2.dylib} & \num{15411424} & 14.70 \\
\texttt{models/cong/base/cong.mdl} & \num{75667563} & 72.16 \\
\texttt{models/cong/base/sj.morph} & \num{8462892} & 8.07 \\
\texttt{models/cong/base/default.dict} & \num{3090954} & 2.95 \\
\texttt{models/cong/base/multi.dict} (optional) & \num{12064440} & 11.50 \\
\midrule
\textbf{Core runtime payload (without \texttt{multi.dict})} & \textbf{\num{102632833}} & \textbf{97.88} \\
\textbf{Core runtime payload (+ \texttt{multi.dict})} & \textbf{\num{114697273}} & \textbf{109.38} \\
\bottomrule
\end{tabular}
\end{table}

For a minimal consumer workflow that calls \texttt{Kiwi::init()}, incremental deployment footprint can be approximated as
\[
\Delta S_{\mathrm{deploy}} \approx \Delta S_{\mathrm{app\_bin}} + S_{\mathrm{runtime\_payload}},
\]
which yields approximately \(0.162\) MiB \(+\) \(97.88\!\sim\!109.38\) MiB on this snapshot, i.e., roughly \(98.0\!\sim\!109.5\) MiB additional footprint for practical runtime use. Bundling static archives or extra model assets increases this bound (full local installation on the measured host: about 194 MiB).
...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \item \textbf{Deployment size is runtime-asset dominated.} In our measurement, Rust consumer binary growth is modest (\(\approx 0.162\) MiB), while practical Kiwi runtime assets contribute roughly \(98\!\sim\!109\) MiB.
\end{enumerate}
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item \textbf{Cross-engine binary identity boundary.} Even with explicit model-path pinning, the packaged native extension in kiwipiepy limits strict binary-level identity with Rust-side dynamic loading paths.
    \item \textbf{Cache interactions.} Varied-input configurations may still retain partial repetition in constrained category pools.
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item \textbf{FFI boundary verification gap.} A full dynamic-analysis matrix is not yet part of the release gate. Planned evidence includes Valgrind or Memcheck runs, sanitizer-gated CI lanes, and scoped Miri checks where applicable.
\end{itemize}

\paragraph{Near-term roadmap.}
\begin{enumerate}[leftmargin=1.5em]
    \item \textbf{Startup optimization track.} Evaluate constructor-path policies for the \texttt{new} and \texttt{from\_config} initialization APIs, plus prewarmed workers and pinning defaults. Include an optional fast-init profile that disables bootstrap/discovery and report median startup deltas on Tier A/Tier B.
    \item \textbf{Memory profiling track.} Add peak RSS and allocation telemetry for repeated, dataset-varied, and cache-minimized modes; publish side-by-side Rust/Python summaries.
    \item \textbf{Cross-host reproducibility track.} Re-run the benchmark matrix on multiple OS/CPU profiles and publish variance envelopes for key features.
    \item \textbf{Parity and artifact hygiene track.} Continue parity-matrix updates, keep skill documentation release-coupled, and publish immutable runtime lock manifests (release tag + binary/model hashes).
    \item \textbf{FFI safety verification track.} Add boundary-focused validation lanes (ASan/UBSan/LSan and Valgrind where supported) plus scoped Miri checks for non-FFI ownership invariants; publish crash-free stress logs and failure triage artifacts.
\end{enumerate}
...
\section{Conclusion}
\texttt{kiwi-rs} demonstrates that a Rust-first integration layer over the Kiwi C API can provide stronger safety and deployment ergonomics than thin bindings while preserving practical performance. The report contributes three concrete artifacts: (i) a capability-aware and ownership-safe runtime wrapper, (ii) a transparent multi-profile benchmark protocol (repeated, cache-active varied, cache-minimized), and (iii) reproducibility-oriented evaluation assets including parity checks, command templates, and appendix algorithms.

Results support strong speedups on selected paths and near-parity or inconclusive outcomes on others, which reinforces a central claim of this work: binding-layer performance must be interpreted by workload mode, not a single headline ratio. Startup overhead and missing memory telemetry remain open operational concerns. With the roadmap outlined in Section~11, \texttt{kiwi-rs} is positioned as a robust engineering baseline for Rust-native Korean NLP services and reproducible systems research.
...
\section{Benchmark and Test Pseudocode}
This appendix summarizes benchmark/test logic in implementation-agnostic pseudocode for reviewability.

\begin{figure}[t]
\centering
\caption{Algorithm 1: Benchmark Compare Loop (Rust vs Python)}
\label{alg:bench_compare}
\begin{algorithmic}[1]
\State \textbf{Input:} dataset \(D\), feature set \(F\), repeats \(n\), warmup \(w\), iterations \(t\)
\ForAll{feature \(f \in F\)}
  \For{run \(=1\) to \(n\)}
    \ForAll{engine in alternating\_order(\{rust, python\})}
      \State warmup(engine, \(f\), \(D\), \(w\))
      \State \(m \gets\) measure\_calls\_per\_sec(engine, \(f\), \(D\), \(t\))
      \State \(s \gets\) sink\_value(engine, \(f\), \(D\))
      \State store(engine, \(f\), run, \(m\), \(s\))
    \EndFor
  \EndFor
  \State \(\rho_f \gets \mathrm{median}(M_{rust,f}) / \mathrm{median}(M_{python,f})\)
  \State run sink-parity guardrail for \(f\)
  \State compute bootstrap CI and \(P(\rho_f>1)\)
\EndFor
\State \textbf{Output:} per-feature metrics, ratios, CI, decisions
\end{algorithmic}
\end{figure}

\begin{figure}[t]
\centering
\caption{Algorithm 2: Sink-Parity Guardrail}
\label{alg:sink_parity}
\begin{algorithmic}[1]
\State \textbf{Input:} sink\(_{rust}\), sink\(_{python}\), threshold \(\epsilon\)
\If{sink\(_{python}=0\)}
  \State \(e \gets |sink_{rust} - sink_{python}|\)
\Else
  \State \(e \gets |sink_{rust} - sink_{python}| / |sink_{python}|\)
\EndIf
\If{\(e > \epsilon\)}
  \State mark feature as non-equivalent; fail strict mode
\EndIf
\end{algorithmic}
\end{figure}
...
\begin{figure}[t]
\centering
\caption{Algorithm 3: Bootstrap Ratio Confidence Interval}
\label{alg:bootstrap_ratio}
\begin{algorithmic}[1]
\State \textbf{Input:} run vectors \(R\), \(P\); bootstrap samples \(B\)
\For{\(b=1\) to \(B\)}
  \State \(R_b \gets\) resample\_with\_replacement(\(R\))
  \State \(P_b \gets\) resample\_with\_replacement(\(P\))
  \State \(\rho_b^* \gets \mathrm{median}(R_b) / \mathrm{median}(P_b)\)
\EndFor
\State \(CI_{95} \gets [Q_{0.025}(\rho^*), Q_{0.975}(\rho^*)]\)
\State \(p \gets \mathrm{mean}(\mathbf{1}[\rho^*>1])\)
\State \textbf{return} \(CI_{95}, p\)
\end{algorithmic}
\end{figure}

\begin{figure}[t]
\centering
\caption{Algorithm 4: Structural Agreement (Cross-Engine)}
\label{alg:struct_agreement}
\begin{algorithmic}[1]
\State \textbf{Input:} per-row Rust outputs \(U_r\), Python outputs \(U_p\)
\ForAll{row \(i\)}
  \State normalize sentence boundaries to character offsets
  \State compare exact token sequence (form, tag, start, end)
  \State compare token-boundary sets (precision/recall/F1)
  \State compare token sets (span+form+tag precision/recall/F1)
  \State compute POS agreement on shared spans
\EndFor
\State aggregate corpus-level rates and confusion pairs
\end{algorithmic}
\end{figure}

\section{Implementation Mapping and Traceability}
\begin{table}[t]
\centering
\caption{Pseudocode-to-implementation mapping in this repository.}
\label{tab:pseudocode_mapping}
\begin{tabular}{P{0.31\linewidth}P{0.24\linewidth}P{0.37\linewidth}}
\toprule
Pseudocode item & Primary implementation path & Role \\
\midrule
Algorithm 1 & \path{scripts/compare_feature_bench.py} & Main benchmark orchestration, ratio/decision reporting, artifact export \\
Algorithm 2 & \path{scripts/compare_feature_bench.py} & Sink-parity calculation, strict/non-strict guardrails \\
Algorithm 3 & \path{scripts/compare_feature_bench.py} & Bootstrap resampling, percentile CI, \(P(\text{ratio}>1)\) \\
Algorithm 4 & \path{scripts/compare_structural_parity.py}, \path{examples/dump_structural_outputs.rs} & Cross-engine structural agreement and confusion extraction \\
\bottomrule
\end{tabular}
\end{table}
...
\section{Build Footprint Command Template}
\begin{verbatim}
cd kiwi-rs
