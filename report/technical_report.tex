...
KOMORAN~\cite{komoran_repo} & Java-based analyzer with dictionary/TRIE-centered design & Open-source Java library & Java API, plus ecosystem wrappers such as PyKOMORAN & JVM-native deployment; custom dictionary workflow emphasized in docs \\
...
Because the Kiwi codebase contains identifiers such as \texttt{TypoTransformer} and \texttt{history\_transformer}, it is easy to misread the engine as Transformer-architecture NLP. Upstream documentation and the project citation, however, describe Kiwi's ambiguity resolution as a combination of a statistical language model and skip-bigram modeling~\cite{kiwi_repo,kiwi_kjdh_2024}. In this report, we therefore treat Kiwi as a statistical morphology analyzer with LM-based disambiguation, while Transformer-family papers are used only as adjacent related-work context.
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item Core LM implementation units are explicitly named \texttt{Knlm}, \texttt{CoNgramModel}, and \texttt{SkipBigramModel}, consistent with statistical LM design.
    \item Build configuration in the upstream project does not introduce deep-learning runtime dependencies (e.g., PyTorch/TensorFlow/ONNX).
\end{itemize}
